...
def load_complete_rental_data(filename=None, max_price_per_sqm=MAX_PRICE_PER_SQM):
    ...
    if filename and os.path.exists(filename):
        logging.info(f"Using provided rental data file: {filename}")
    else:
        ...
        possible_files = [
            f"propbot/data/processed/rental_data_{current_month}.csv",  # New directory structure
            f"rental_data_{current_month}.csv",                         # Current month
            f"rental_data_{current_month}_complete.csv",                # Current month (complete)
            "propbot/data/processed/rental_data.csv",                   # Generic in new structure
            "rental_data.csv",                                          # Generic fallback
            "rental_complete.csv"                                       # Legacy fallback
        ]
        ...
        with open(filename, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                ...
                if row[2]:  # rent_price column
                    price_match = re.search(r'(\d+(?:,\d+)?)', row[2].replace('.', ''))
                    if price_match:
    ...
    for property_item in sales_data:
        ...
        if comparable_count < min_comparables:
            ...
            if price_per_sqm_values:
                ...
                # Calculate monthly rent based on property size
                if property_size > 0:
                    # FIXED CALCULATION: Use price per sqm × property size to get monthly rent
                    # For Lisbon, a reasonable price per sqm for rental is typically 1-1.5% of the purchase price per sqm
                    purchase_price_per_sqm = property_price / property_size
                    # For rental properties, we expect 2.5-3.5% of the purchase price as monthly rent
                    rental_yield_percentage = 2.5  # Using 2.5% as a realistic estimate for Lisbon market
                    monthly_rent = (purchase_price_per_sqm * rental_yield_percentage / 100) * property_size
                    
                    # Ensure minimum rent and check against actual comps
                    ...
                    # Apply a maximum cap to ensure realistic values
                    max_monthly_rent = property_price * 0.008  # Maximum 0.8% of property price per month
                    if monthly_rent > max_monthly_rent:
                        monthly_rent = max_monthly_rent
...
def extract_size(details_text):
    ...
    # Handle cases like "T275 m²" where room type and size are combined
    room_size_match = re.search(r'T(\d+)\s*m²', details_text)
    if room_size_match:
        room_size_str = room_size_match.group(1)
        
        # For rental listings with format "T146 m²", first digit is often room type
        # and remaining digits are the size
        if len(room_size_str) >= 3:
            return float(room_size_str[1:])
        
    # Try to match the room type pattern without m² for cases missing the unit
    room_size_match = re.search(r'T(\d+)', details_text)
    if room_size_match and len(room_size_match.group(1)) > 1:
        room_size_str = room_size_match.group(1)
        
        # For T2XX pattern, extract XX as the size
        if len(room_size_str) == 3 and room_size_str.startswith('2'):
            return float(room_size_str[1:])
            
        # For other cases like T150, treat the last two digits as the size
