...
def load_sales_data(filename="idealista_listings.json"):
    """
...
def extract_size(details):
    """Extract property size in square meters from details string."""
    if not details:
        return None
    
    # Handle the case where format is "T[room_number][size] m²"
    # Example: "T260 m²" should extract 60 m²
    special_format_match = re.search(r'T[0-4](\d+)\s*m²', details)
    if special_format_match:
        try:
            return float(special_format_match.group(1))
        except (ValueError, TypeError):
            return None
    
    # Regular case with patterns like "46 m²" or "46m²"
...
def load_rental_data(filename):
    """Load rental data from CSV file and extract relevant information."""
