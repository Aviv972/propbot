...
def load_investment_data(report_file: Optional[Union[str, Path]] = None) -> List[Dict[str, Any]]:
    """
    Load investment data from the database or a report file.
    
    Args:
        report_file: Path to investment report JSON file (optional)
        
    Returns:
        List of property dictionaries with investment metrics
    """
    try:
        # First try to load from database
        properties = get_analyzed_properties()
        
        if properties:
            logger.info(f"Loaded {len(properties)} analyzed properties from database")
            return properties
            
        # Fallback to file if specified and database has no data
        if report_file:
            # Load from file
            ...
            logger.info(f"Loaded {len(properties)} properties from file: {report_file}")
            return properties
...
def generate_html_dashboard(investment_data):
    ...
    try:
        ...
        # Check if all properties have no rental data
        all_properties_without_rental = all(prop.get('monthly_rent', 0) <= 0 for prop in investment_data)
        
        # Calculate averages only if we have valid data
        if not all_properties_without_rental:
            avg_price = sum(prop.get('price', 0) for prop in investment_data) / total_properties if total_properties > 0 else 0
            avg_size = sum(prop.get('size', 0) for prop in investment_data) / total_properties if total_properties > 0 else 0
            avg_gross_yield = sum(prop.get('gross_yield', 0) for prop in investment_data) / total_properties if total_properties > 0 else 0
            avg_cap_rate = sum(prop.get('cap_rate', 0) for prop in investment_data) / total_properties if total_properties > 0 else 0
            avg_cash_on_cash = sum(prop.get('cash_on_cash', 0) for prop in investment_data) / total_properties if total_properties > 0 else 0
        else:
            # When all properties lack rental data, use these placeholder values
            avg_price = sum(prop.get('price', 0) for prop in investment_data) / total_properties if total_properties > 0 else 0
            avg_size = sum(prop.get('size', 0) for prop in investment_data) / total_properties if total_properties > 0 else 0
            avg_gross_yield = "N/A"
            avg_cap_rate = "N/A"
            avg_cash_on_cash = "N/A"
        
        # Format currency values with euro symbol and thousands separator
        formatted_avg_price = f"€{avg_price:,.0f}" if isinstance(avg_price, (int, float)) else avg_price
        formatted_avg_size = f"{avg_size:.1f} m²" if isinstance(avg_size, (int, float)) else avg_size
        formatted_avg_yield = f"{avg_gross_yield:.2f}%" if isinstance(avg_gross_yield, (int, float)) else avg_gross_yield
        formatted_avg_cap_rate = f"{avg_cap_rate:.2f}%" if isinstance(avg_cap_rate, (int, float)) else avg_cap_rate
        formatted_avg_coc = f"{avg_cash_on_cash:.2f}%" if isinstance(avg_cash_on_cash, (int, float)) else avg_cash_on_cash
        
        # Get rental data update information from database
        rental_last_update = get_rental_last_update()
        rental_update_frequency = get_rental_update_frequency()
        
        if rental_last_update:
            rental_last_update_str = rental_last_update.strftime("%Y-%m-%d")
        else:
            rental_last_update_str = "Unknown"
            
        rental_update_frequency_str = f"{rental_update_frequency} days"
        
        # Get neighborhoods
        neighborhoods = sorted(set(prop.get('neighborhood', '') for prop in investment_data if prop.get('neighborhood')))
        
        # Get room types
        room_types = sorted(set(prop.get('rooms', '') for prop in investment_data if prop.get('rooms') is not None))
        ...
        table_rows = ""
        for i, prop in enumerate(investment_data):
            ...
            <div class="container">
                ...
                <div class="dashboard-section">
                    ...
                    <div class="summary-cards">
                        <div class="card">
                            ...
                            <div class="card-value">{formatted_avg_price}</div>
                        </div>
                        <div class="card">
                            <div class="card-title">Average Property Size</div>
                            <div class="card-value">{formatted_avg_size}</div>
                        </div>
                        <div class="card">
                            <div class="card-title">Average Gross Yield</div>
                            <div class="card-value">{formatted_avg_yield}</div>
                        </div>
                        <div class="card">
                            <div class="card-title">Average Cap Rate</div>
                            <div class="card-value">{formatted_avg_cap_rate}</div>
                        </div>
                        <div class="card">
                            <div class="card-title">Average Cash on Cash Return</div>
                            <div class="card-value">{formatted_avg_coc}</div>
                        </div>
                    </div>
                ...
                <div class="dashboard-section">
                    ...
                    <div class="timestamp">
                        ...
                        <br>
                        <span>Rental data last updated: {rental_last_update_str} (updates every {rental_update_frequency_str})</span>
                    </div>
                </div>
            </div>
            <div class="navbar">
